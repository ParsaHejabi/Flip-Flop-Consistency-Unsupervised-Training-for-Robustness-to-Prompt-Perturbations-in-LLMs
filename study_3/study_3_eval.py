#!/usr/bin/env python3
"""Evaluation runner for study_3 abl_np transfer experiments."""
...
STUDY3_DATA_ROOT = ROOT_DIR / "study_3" / "data"
...
try:
    ...
    raise ImportError("study_3 requires the `evaluate` package. Install it before running.") from exc
...
LOGGER = logging.getLogger("study_3_eval")
...
def _load_dataset_from_cache(
    ...
    cache_root = data_root / model_identifier / dataset_id / f"abl_np{eval_abl_np}"
    serialized_path = cache_root / "test.pt"
    LOGGER.info(f"Loading cached dataset from {serialized_path}")
    if not serialized_path.exists():
        raise FileNotFoundError(f"Expected cached dataset at {serialized_path}. Re-run the study_3 data generation script for abl_np={eval_abl_np}.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run study_3 abl_np transfer evaluation.")
    ...
    parser.add_argument(
        "--output_dir",
        required=True,
        help="Base output directory (will create results/<dataset>_abl_npX/accuracy_<dataset>_abl_npY).",
    )
    ...
    parser.add_argument("--best_ckpt", required=True, help="Path to the best checkpoint directory from the abl_np=training run.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="abl_np used for the training run whose checkpoint is evaluated (default: 5).",
    )
    parser.add_argument(
        "--eval_abl_np",
        type=int,
        default=10,
        help="abl_np associated with the cached test split to evaluate on (default: 10).",
    )
    parser.add_argument(
        "--data_root",
        default=str(STUDY3_DATA_ROOT),
        help="Root directory containing serialized study_3 datasets (default: study_3/data).",
    )
...
def main() -> None:
    ...
    LOGGER.info(
        "Starting study_3 evaluation (train abl_np=%d -> eval abl_np=%d)",
        args.train_abl_np,
        args.eval_abl_np,
    )
    ...
    LOGGER.info("Using user-provided best checkpoint from abl_np=%d run: %s", args.train_abl_np, best_ckpt)
    ...
    if args.datasets:
        dataset_labels_raw = list(args.datasets)
        LOGGER.info("User-provided datasets: %s", ", ".join(dataset_labels_raw))
    else:
        dataset_labels_raw = list(DEFAULT_DATASETS)
        LOGGER.info("No datasets specified; defaulting to: %s", ", ".join(dataset_labels_raw))

    dataset_order: List[str] = []
    for label in dataset_labels_raw:
    ...
    for dataset_label in dataset_order:
        ...
        dataset_train_dir = results_root / f"{dataset_label}_abl_np{args.train_abl_np}"
        dataset_out_dir = dataset_train_dir / f"accuracy_{dataset_label}_abl_np{args.eval_abl_np}"
        ...
        try:
            ...
            LOGGER.info(
                "[%s] Loaded cached dataset (abl_np=%d) from %s",
                dataset_label,
                args.eval_abl_np,
                serialized_path,
            )
        ...
        summary[dataset_label] = {
            "abl_np_train": args.train_abl_np,
            "abl_np_eval": args.eval_abl_np,
            "metrics": numeric_metrics,
        }
        LOGGER.info("[%s] Metrics: %s", dataset_label, numeric_metrics)

    summary_path = results_root / f"study_3_metrics_abl_np{args.train_abl_np}_vs_abl_np{args.eval_abl_np}.json"
